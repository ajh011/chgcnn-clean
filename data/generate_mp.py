...
class CrystalHypergraphDataset(Dataset):
    ...
    def __getitem__(self, index, report = True):
        mp_id, band, form, hull, metal = self.id_prop_data[index]
        ...
        hgraph = Crystal_Hypergraph(struc, mp_id=mp_id, target_dict={'band_gap':torch.tensor(float(band)),
                                                                     'form_en': torch.tensor(float(form)),
                                                                     'en_hull': torch.tensor(float(hull)),
                                                                     'metalicity': torch.tensor(int(metal))})
        hgraph.struc = None
        if report:
