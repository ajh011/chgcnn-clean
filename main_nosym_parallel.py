...
def train_worker(rank, model, train_loader, loss_criterion, accuracy_criterion, optimizer, epoch, task, target_name, normalizer, scheduler):
    ...
    for i, data in enumerate(train_loader):
        ...
        if task == 'regression':
            target = torch.tensor([float(i) for i in data[target_name]]).to(rank)
...
def validate_worker(rank, model, test_loader, loss_criterion, accuracy_criterion, epoch, task, target_name, normalizer):
    batch_time = AverageMeter('Batch', ':.4f')
    losses = AverageMeter('Loss', ':.4f')
    accus = AverageMeter('Accu', ':.4f')
    progress = ProgressMeter(
...
def main_worker(rank, world_size, test_arg):
    parser = argparse.ArgumentParser(description='CHGCNN')
    ...
    parser.add_argument('--weight-decay', default=0, type=float,
                        metavar='W', help='weight decay (default: 0 was 5e-5)',
                        dest='weight_decay')
    ...
    #### Set device & model and try to parallelize
    model = CrystalHypergraphConv(classification = class_bool)
    model.to(rank)
    ...
    if args.scheduler == True:
        ...
        scheduler = MultiStepLR(optimizer, milestones = [250,500,750], gamma=0.5)
    ...
    for epoch in range(args.start_epoch, args.epochs + 1):
        train_loss, train_accu = train_worker(rank, model, train_loader, loss_criterion, accuracy_criterion, optimizer,
                                       epoch, args.task, 'target', normalizer, scheduler)
        val_accu = validate_worker(rank, model, val_loader, loss_criterion, accuracy_criterion, epoch, args.task, 'target', normalizer)
